...
export function SortableMentorRow({ mentor, onEdit, onDelete }: SortableRowProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    <TableRow ref={setNodeRef} style={style}>
      ...
      <TableCell className="w-[80px]">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-50 border overflow-hidden">
          {mentor.image_url ? (
            <img
              src={resolveBackendMediaUrl(mentor.image_url)}
              alt={mentor.name}
      ...
      <TableCell className="font-bold text-slate-900">{mentor.name}</TableCell>
      <TableCell className="text-slate-600">{mentor.title}</TableCell>
      <TableCell className="text-slate-500 max-w-[180px] truncate">{mentor.organization || "-"}</TableCell>
      <TableCell>
        {mentor.linked_in ? (
          <a
      ...
      <TableCell className="text-center font-semibold text-slate-700">{mentor.display_order}</TableCell>
      <TableCell className="text-right">
        <div className="flex justify-end gap-1">
          <Button variant="ghost" size="icon" onClick={() => onEdit(mentor)} className="cursor-pointer">
            <Edit2 className="h-4 w-4 text-slate-500 hover:text-[#FF6B00]" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => onDelete(mentor.id)} className="cursor-pointer">
