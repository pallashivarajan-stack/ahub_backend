...
export function SortableTeamMemberRow({ member, onEdit, onDelete }: SortableRowProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <TableRow ref={setNodeRef} style={style}>
      ...
      <TableCell className="text-slate-500 max-w-[200px] truncate">{member.tagline || "-"}</TableCell>
      <TableCell>
        {member.visit_link ? (
          <a
            href={member.visit_link}
            target="_blank"
